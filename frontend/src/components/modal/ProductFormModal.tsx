...
const ProductFormModal: React.FC<ProductFormModalProps> = ({
  ...
  >({
    ...
    amount: 0,
    sellPrice: 0,
    weigth: 0, // Delete when backend deletes it
    buyDate: '',
    expiration: '',
    providerIds: [],
  });
    ...
    if (isOpen) {
      ...
      if (product) {
        ...
        setFormData({
          ...
          weigth: product.weigth || 0, // Delete when backend deletes it
  ...
  // Para modo edición, también verificar si se ha tocado algún campo
  ...
  const handleSave = () => {
    ...
    const productToSave: ProductWithOptionalId = {
      ...
      weigth: Number(formData.weigth) || 0.01, // Delete when backend deletes it
...
//   category?: string;
//   sellPrice?: string;
//   // weigth?: string; // Delete when backend deletes it
//   expiration?: string;
// }
...
//           newErrors.description = "La descripción del producto es obligatoria.";
//         } else if (
//           stringValue.trim().length < 10 ||
//           stringValue.trim().length > 255
//         ) {
//           newErrors.description =
//             "La descripción del producto debe tener entre 10 y 255 caracteres.";
//         } else {
...
//     if (
//       ["name", "description", "category", "sellPrice"].includes(field)
//     ) {
//       setTimeout(() => {
//         validateField(field as ValidatableFields, value);
//       }, 100);
...
//       "category",
//       "sellPrice",
//       // "weigth", // Delete when backend deletes it
//     ];
//     let isValid = true;
...
//     formData.category.trim() &&
//     formData.sellPrice > 0;
//     // formData.weigth > 0; // Delete when backend deletes it
...
//                 onClose();
//               }}
//               className="absolute top-4 right-4 text-blue-ocean hover:text-gray-600 transition-colors cursor-pointer"
//             >
//               <X size={22} />
//             </button>
//           </header>
...
//               {/* Delete when backend deletes weight/weigth field */}
//               {/* <div>
//                 <label className="block text-sm font-medium text-dark-blue mb-2">
//                   Peso (kg)
//                 </label>
