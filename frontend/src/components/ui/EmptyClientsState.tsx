...
const EmptyClientsState: React.FC<{ onAddClient: () => void }> = ({ onAddClient }) => {
  return (
    <article className="flex min-h-96 flex-col items-center justify-center py-12 text-center">
      <div className="mb-4 rounded-full bg-gray-100 p-6">
        <Users size={48} className="text-gray-400" />
      </div>
      <h3 className="mb-2 text-xl font-medium text-gray-900">Aún no tienes clientes registrados</h3>
      <p className="mb-6 max-w-md text-gray-500">
        Comienza agregando tu primer cliente para gestionar tu cartera y operaciones de manera
        eficiente.
      </p>
      <button
        onClick={onAddClient}
