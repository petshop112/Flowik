...
const Home = () => {
  ...
  // ordering the products para renderizar solo los más vendidos
  const orderedProducts = [...productosData].sort((a, b) => b.ventas - a.ventas);

  // filtrando data de la db para stock bajo y top productos
  const topStockBajo = [...products].sort((a, b) => a.amount - b.amount).slice(0, 10);

  const topProductosData = orderedProducts.slice(0, 7);

  // seleccionando productos para mostrar debajo del gráfico de rotación
  const activeProd =
                ...
                >
                  ...
                  >
                    {topProductosData.map((entry, idx) => (
                      <Cell
                        key={`cell-${idx}`}
                        fill={entry.producto === activeProd.producto ? '#2563eb' : '#BBD7FF'}
                        cursor="pointer"
            ...
            <table className="w-full">
              ...
              <tbody className="bg-white">
                <tr>
                  ...
                  <td className="px-4 py-4 text-sm">Disponibles en stock (uds)</td>
